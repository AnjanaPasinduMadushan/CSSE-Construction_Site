...
import { editConstructionSite, getAllSiteManagers } from '../../api/services/siteService';
...
function EditSite() {
    ...
    const decryptedData = AES.decrypt(decodeURIComponent(rowData), 'yourSecretKey').toString(enc.Utf8);
    ...
    const parsedRowData = JSON.parse(decryptedData);

    // console.log("parsedRowData", parsedRowData)

    const [siteName, setsiteName] = useState(parsedRowData.siteName || '');
    const [siteManager, setsiteManager] = useState(parsedRowData.managerID || '');
    const [threshold, setThreshold] = useState(parsedRowData.threeshold || '');
    const [description, setDescription] = useState(parsedRowData.description || '');
    const [googleMap, setgoogleMaps] = useState(parsedRowData.location || '');
    const [province, setProvince] = useState(parsedRowData.province || '');
    const [startDate, setStartDate] = useState(parsedRowData.startDate ? dayjs(parsedRowData.startDate) : null);
    const [endDate, setEndDate] = useState(parsedRowData.endDate ? dayjs(parsedRowData.endDate) : null);
    const [address, setAddress] = useState(parsedRowData.address || '');
    ...
    const handleSiteSubmit = async (e) => {
        ...
        try {
            ...
            const response = await editConstructionSite(
                parsedRowData.siteID,
                siteName,
                googleMap,
                threshold,
                address,
